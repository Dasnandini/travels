...
export default function RouteDetailPageContent({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  const handleEditSubmit = async (values: any) => {
    ...
    router.push(`/admin/operations/routes/${id}`);
  };
    ...
    return (
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="border-b border-slate-800/80 pb-6">
          <Link
            href={`/admin/operations/routes/${id}`}
        ...
        <RouteForm
          ...
          onCancel={() => router.push(`/admin/operations/routes/${id}`)}
  ...
  return (
    <RouteDetails
      route={route}
      onEdit={() => router.push(`/admin/operations/routes/${id}?edit=true`)}
